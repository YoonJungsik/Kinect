...
namespace KinectExile.Models
{
    ...
    public struct RingBuffer
    {
        ...
        public byte get_rgb_frame(int frameNo, int offset)
        {
            if (_data == null) return 0;
            return _data[get_index(frameNo)].ColorPixelData[offset];
        }

        public short get_depth_frame(int frameNo, int offset)
        {
            if (_data == null) return 0;
            return _data[get_index(frameNo)].DepthPixelData[offset];
        }


        public double get_depth_length(int frameNo, int offset)
        {
            if (_data == null) return 0;
            return _data[get_index(frameNo)].DepthLengthData[offset];
        }
        ...
        public bool init_ringbuffer(double buffer_sec, int ColorStream_FramePixelDataLength, int DepthStream_FramePixelDataLength, out string ret_message)
        {
            ...
            {
                buffer_index = 0;
                max_buffer_frame = (int)(30 * Math.Ceiling(buffer_sec)+1); //30fps*second
        ...
        public void save_framedata(byte[] value)
        {
        ...
        public void save_depthLength(double[] value)
        {
